...
import 'fleche_ae.dart'; // Importation du fichier fleche.dart
...
class _ApercuState extends State<Apercu> {
  ...
  void initState() {
    super.initState();
    aspectsEnvironnementauxFuture = fetchAspectsEnvironnementaux();
  }
  ...
  void _showEditDialog(Map<String, dynamic> aspectEnvironnemental) {
    final TextEditingController _controller = TextEditingController(text: aspectEnvironnemental['libelle']);
                ...
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      ElevatedButton(
                        onPressed: () async {
                          await updateAspectEnvironnemental(aspectEnvironnemental['id_aspect_env'], _controller.text);
                          setState(() {
                            aspectsEnvironnementauxFuture = fetchAspectsEnvironnementaux();
                          });
                ...
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      ElevatedButton(
                        onPressed: () async {
                          await deleteAspectsEnvironnemental(id);
                          setState(() {
                            aspectsEnvironnementauxFuture = fetchAspectsEnvironnementaux();
                          });
              ...
              children: [
                ...
                Expanded(
                  child: FutureBuilder<List<Map<String, dynamic>>>(
                    future: aspectsEnvironnementauxFuture,
                    builder: (context, snapshot) {
                      if (snapshot.connectionState == ConnectionState.waiting) {
                        ...
                        final aspectsEnvironnementaux = snapshot.data!;

                        return ListView.builder(
                          itemCount: aspectsEnvironnementaux.length,
                          itemBuilder: (context, index) {
                            final aspectEnvironnemental = aspectsEnvironnementaux[index];
                                ...
                                child: ListTile(
                                  title: Row(
                                    children: [
                                      Expanded(
                                        child: CustomText(
                                          text: aspectEnvironnemental['libelle'],
                                          size: 15,
                                        ),
                                      ),
                                      IconButton(
                                        icon: const Icon(Icons.edit, color: Colors.blue),
                                        onPressed: () => _showEditDialog(aspectEnvironnemental),
                                      ),
                                      IconButton(
                                        icon: const Icon(Icons.close, color: Colors.red),
                                        onPressed: () => _showDeleteConfirmationDialog(aspectEnvironnemental['id_aspect_env']),
                                      ),
                                    ],
                                  ),
                                ),
